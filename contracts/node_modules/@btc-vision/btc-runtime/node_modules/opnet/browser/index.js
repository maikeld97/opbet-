  ...
  constructor(transaction, network) {
    ...
    try {
      this.from = new Address(
        fromBase64(transaction.from),
        fromBase64(transaction.fromLegacy)
      );
      this.contractAddress = transaction.contractAddress;
      this.contractPublicKey = new Address(
        fromBase64(transaction.contractPublicKey)
      );
...
class CallResultSerializer {
  ...
  static networkNameToU8(network) {
    switch (network) {
      ...
      case "regtest" /* Regtest */:
        return 2;
      case "opnetTestnet" /* OpnetTestnet */:
        return 3;
      default:
        return 2;
    }
  ...
  static u8ToNetworkName(value) {
    switch (value) {
      ...
      case 3:
        return "opnetTestnet" /* OpnetTestnet */;
      default:
        return "regtest" /* Regtest */;
    }
  }
...
class ContractData {
  ...
  constructor(raw) {
    ...
    this.contractPublicKey = raw.contractPublicKey instanceof Uint8Array ? new Address(raw.contractPublicKey) : new Address(fromBase64(raw.contractPublicKey));
    this.bytecode = raw.bytecode instanceof Uint8Array ? raw.bytecode : fromBase64(raw.bytecode);
    ...
    this.deployerPubKey = raw.deployerPubKey instanceof Uint8Array ? raw.deployerPubKey : fromBase64(raw.deployerPubKey);
    ...
    this.deployerHashedPublicKey = raw.deployerAddress instanceof Address ? fromHex(cleanDeployerAddr) : fromBase64(cleanDeployerAddr);
    this.contractSeed = raw.contractSeed instanceof Uint8Array ? raw.contractSeed : fromBase64(raw.contractSeed);
    this.contractSaltHash = raw.contractSaltHash instanceof Uint8Array ? raw.contractSaltHash : fromBase64(raw.contractSaltHash);
    if (this.deployerHashedPublicKey && this.deployerPubKey) {
...
class StoredValue {
  ...
  constructor(iStoredValue) {
    ...
    if (typeof iStoredValue.value !== "string") {
      ...
      this.value = iStoredValue.value.startsWith("0x") ? fromHex(iStoredValue.value.slice(2)) : fromBase64(iStoredValue.value);
    }
    ...
    const payload = this.buildJsonRpcPayload(JSONRpcMethods.SUBMIT_EPOCH, [
      {
        ...
        graffiti: params.graffiti ? toHex(params.graffiti) : void 0
      }
    ]);
    ...
    return {
      inputs: transaction.inputs.map((input) => {
        return {
          ...
          coinbase: input.coinbase ? toBase64(input.coinbase) : void 0,
          flags: input.flags
        };
      }),
      outputs: transaction.outputs.map((output) => {
        return {
          ...
          scriptPubKey: output.scriptPubKey ? toBase64(output.scriptPubKey) : void 0,
...
class BaseThreader extends Logger {
  ...
  bindCleanupHandlers() {
    ...
    } else if (isReactNative) ; else if (typeof window !== "undefined" && typeof window.addEventListener === "function") {
...
class IBaseContract {
  ...
  encodeInput(writer, abi, value) {
    ...
    if (isAbiTuple(type)) {
      ...
      if (type.length === 1 && firstType !== void 0) {
...
function isP2MRAddress(addr, network) {
  try {
    ...
    return decoded.prefix === network.bech32 && decoded.version === P2MR_WITNESS_VERSION && decoded.data.length === P2MR_DATA_LENGTH;
  } catch {
    return false;
  }
}
...
export { ABIDataTypes, AbiTypeToStr, AbstractRpcProvider, AuthError, BaseContract, BitcoinAbiTypes, BitcoinInterface, BitcoinUtils, Block, BlockGasParameters, BlockWitness, BlockWitnessAPI, CallResult, CallResultSerializer, ConnectionState, ContractData, DEFAULT_CONFIG, DeploymentTransaction, EXTENDED_OP721_ABI, EXTENDED_OP721_EVENTS, Epoch, EpochDifficultyConverter, EpochMiner, EpochSubmission, EpochTemplate, EpochWithSubmissions, ErrorMessages, GenericTransaction, IBaseContract, InteractionTransaction, InternalError, JSONRPCErrorCode, JSONRPCErrorHttpCodes, JSONRpcMethods, JSONRpcProvider, MOTOCHEF_ABI, MOTOSWAP_ROUTER_ABI, MOTOSWAP_STAKING_ABI, MOTO_ABI, MempoolDeploymentTransactionData, MempoolGenericTransactionData, MempoolInteractionTransactionData, MempoolOPNetTransactionData, MempoolTransactionData, MempoolTransactionParser, MotoSwapFactoryAbi, MotoSwapFactoryEvents, MotoSwapPoolEvents, MotoswapPoolAbi, MultiOracleStablecoinEvents, MultiOracleStablecoin_ABI, NativeSwapAbi, NetworkName, OP20Events, OP721Events, OPNetEvent, OPNetTransactionTypes, OP_20S_ABI, OP_20_ABI, OP_721_ABI, OP_NET_ABI, P2MR_MS, P2TR_MS, PeggedTokenEvents, PeggedToken_ABI, ProtocolError, ResourceError, STAKING_ABI, StableCoinEvents, StableCoin_ABI, StoredValue, SubmissionStatus, SubmittedEpoch, SubscriptionType, TransactionBase, TransactionInput, TransactionInputFlags, TransactionOutput, TransactionOutputFlags, TransactionParser, TransactionReceipt, UTXO, UTXOsManager, ValidationError, WebSocketClientEvent, WebSocketRequestOpcode, WebSocketResponseOpcode, WebSocketRpcProvider, decodeRevertData, getConnectionStateName, getContract, getErrorMessage, getSubscriptionTypeName, isP2MRAddress, opnet, parseBlockWitnesses, stringBase64ToBuffer, stringToBuffer, version };
